...
**Current milestone: [Milestone 10 — Enemy economy and repeat assaults](docs/milestone-10.md) is COMPLETE** in `res://scenes/economy_assault.tscn`, with all eight feature acceptance groups passing automated validation. The completed matrix passed **59/60 executions, 7,393 assertions with 3 failures, phase exit 1**; graphical `choke_30` movement-stress attribution remains **unknown**. The opponent and zero-credit two-wave integration passed both modes (261 assertions, exits 0). [Milestone 9 — Attack-move commands](docs/milestone-9.md) and [Milestone 8.1 — HUD clarity](docs/milestone-8.1.md) remain complete. Documented movement limitations remain deferred and unresolved; full movement/Milestone 5 regression acceptance has not passed. The [current roadmap](docs/roadmap.md) keeps those limitations separate from the new unknown-attribution failure. No human playtest occurred. Milestone 11 is not started.
