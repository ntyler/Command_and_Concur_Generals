...
**Current milestone (2026-09-07): [Milestone 8.1 — HUD clarity and contextual selection](docs/milestone-8.1.md)** is complete in the existing combined-arms assault scene. The M8.1 report records post-fix validation, reused unaffected results, rendered captures and all eight acceptance groups. Outstanding movement failures are **deferred known limitations, still unresolved**, and do not automatically block feature development. Full movement/Milestone 5 regression acceptance has not passed. The [current roadmap](docs/roadmap.md) preserves earlier repairs, tests and historical evidence while assessing each feature separately.
