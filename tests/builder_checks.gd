...
func _builder_placement() -> void:
	...
	_check(not builders.placement.active and builders.manual_pause_active and actor.order_version == order and actor.assigned_destination == destination, "Escape preview cancellation pauses and preserves the accepted earlier order")
	await _hud_key(KEY_ESCAPE)
	_check(not builders.manual_pause_active, "Escape resumes the retained builder order")
...
func _builder_ui() -> void:
	...
	await _hud_key(KEY_ESCAPE)
	_check(not builders.help_panel.is_open() and not builders.placement.active and builders.manual_pause_active, "one Escape closes Help and the free builder preview and opens Pause")
	await _hud_key(KEY_ESCAPE)
	await _hud_key(KEY_3)
	_check(not builders.manual_pause_active and not builders.placement.active and not builders.selection.placement_active and builders.selection.selected_units() == army and builders.control_groups.group_members(3) == army and army.all(func(unit: RTSUnit) -> bool: return unit.selection_indicator.visible) and not actor.selection_indicator.visible and not panel.depot_button.visible, "second Escape resumes and 3 restores visible army without builder context")
	await _hud_key(KEY_Q)
	_check(builders.selection.attack_move_targeting, "recalled army Q works after Help and builder placement cancellation")
	await _hud_key(KEY_ESCAPE)
	_check(builders.manual_pause_active and not builders.selection.attack_move_targeting, "Escape pauses and cancels recalled army targeting")
	await _hud_key(KEY_ESCAPE)
	await _hud_pick_unit(actor)
	await _click(panel.depot_button.get_global_rect().get_center(), MOUSE_BUTTON_LEFT)
	_motion(_world_screen(DEPOT_POINT))
