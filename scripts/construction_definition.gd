...
func is_valid() -> bool:
	var supported := kind in [RTSBuilding.Kind.BARRACKS, RTSBuilding.Kind.VEHICLE_FACTORY, RTSBuilding.Kind.SUPPLY_DEPOT, RTSBuilding.Kind.POWER_PLANT, RTSBuilding.Kind.GROUND_DEFENSE_BATTERY]
	var defense_valid := kind != RTSBuilding.Kind.GROUND_DEFENSE_BATTERY or (weapon_data != null and weapon_data.is_valid() and weapon_data.mode == WeaponDefinition.Mode.HITSCAN and is_finite(acquisition_interval) and acquisition_interval > 0.0 and is_finite(turret_turn_speed) and turret_turn_speed > 0.0)
