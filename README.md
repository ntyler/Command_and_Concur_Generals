...
**Current milestone: [Milestone 12 — Bulldozers and builder-driven construction](docs/milestone-12.md)** is implemented in `res://scenes/builder_assault.tscn`; final validation is in progress. This is the intended builder-based player experience. The source-matched [Milestone 11 baseline](docs/milestone-11.md) remains complete for its feature requirements, with 63/64 matrix executions passing and a retained unknown-attribution projection failure. Full movement/Milestone 5 acceptance remains unresolved. No human playtest occurred. Power generation is scheduled afterward and is not implemented.
...
The [Milestone 12 report](docs/milestone-12.md) records builder acceptance and every retained execution. `tools/validate-m12.ps1 -RunName <unique-phase>` runs both builder suites plus the complete M11 regression matrix in headless and graphical modes on a fresh source-hashed copy through the existing external timeout wrapper. The [Milestone 11 report](docs/milestone-11.md) preserves depot acceptance, the corrected return comparison, source correspondence, failed phases and viewport evidence used as the starting baseline.
