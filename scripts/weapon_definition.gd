...
func is_valid() -> bool:
	if mode != Mode.HITSCAN and mode != Mode.GUIDED_PROJECTILE:
		return false
	return is_finite(damage) and damage > 0.0 and is_finite(attack_range) and attack_range > 0.0 and is_finite(cooldown) and cooldown > 0.0 and is_finite(facing_tolerance_degrees) and facing_tolerance_degrees >= 0.0 and (mode == Mode.HITSCAN or (is_finite(projectile_speed) and projectile_speed > 0.0 and is_finite(projectile_lifetime) and projectile_lifetime > 0.0))
