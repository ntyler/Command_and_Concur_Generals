...
func _fort_normal_ui() -> void:
	...
	await _frames(2)
	_check(not fort.placement.status.visible and not fort.placement.preview.visible, "pointer over orientation control hides stale world-preview feedback")
	for dimensions in [Vector2i(1280, 720), Vector2i(1920, 1080)]:
		await _hud_layout(dimensions)
		_motion(_world_screen(FORT_WALL_POINT))
		await _frames(10)
		# Barrier snapping constrains x/z; the physics ray supplies the ground y.
		print("WALL_PREVIEW: point=(%.12f, %.12f, %.12f) valid=%s" % [fort.placement.point.x, fort.placement.point.y, fort.placement.point.z, fort.placement.valid])
		_check(fort.placement.valid and fort.placement.preview.visible and fort.placement.status.visible and fort.placement.status.text.contains("Valid") and fort.placement.status.text.contains("0°") and Vector2(fort.placement.point.x, fort.placement.point.z) == Vector2(FORT_WALL_POINT.x, FORT_WALL_POINT.z), "current snapped wall preview shows valid full-footprint placement and matching orientation")
		await _fort_capture("fortification_valid_wall_preview_%dx%d" % [dimensions.x, dimensions.y])
		_motion(_world_screen(Vector3(-10, 0, -12)))
		await _frames(10)
		_check(not fort.placement.valid and fort.placement.preview.visible and fort.placement.status.visible and fort.placement.status.text.contains("Cannot build") and not fort.placement.blocking_regions.is_empty() and fort.placement.conflict_indicator.visible, "current invalid wall preview identifies and highlights actual protected access")
		await _fort_capture("fortification_protected_wall_preview_%dx%d" % [dimensions.x, dimensions.y])
	root.size = Vector2i(1280, 720)
	await _frames(4)
