...
**Outstanding movement issues are DEFERRED KNOWN LIMITATIONS; their engineering status remains UNRESOLVED. They do not automatically block authorized feature development, currently Milestone 8. Full movement acceptance and full Milestone 5 regression acceptance have not passed.** This is a scheduling decision, not a bug closure, test waiver or claim of movement reliability.
...
| **8 — tactical interface; validation in progress** | **A compact fully revealed minimap and keyboard groups 1–9 in the existing combined-arms scene. Shared map conversion, live entity markers/camera footprint, minimap camera/Move input, group assignment/recall/double-tap and match-scoped lifecycle. See the [M8 report](milestone-8.md) for actual acceptance and validation results.** |
...
The current user-authorized work is **Milestone 8 — tactical minimap and control groups** in `res://scenes/combined_arms_assault.tscn`. Left-click the minimap to center the existing camera; right-click for ordinary ground Move. **Ctrl + 1–9** replaces/clears a mobile-unit group, **1–9** recalls it, and a double-tap centers the camera. Marker refresh defaults to **0.1 seconds**, and double-tap timing to **0.3 seconds** of real input time. The map uses fixed +X-right/+Z-down orientation and shows the fully revealed field. Existing selection, ownership, command acceptance, construction, production and match-result/Restart systems remain authoritative.

M8 validation is in progress and recorded in its [acceptance report](milestone-8.md), including viewport input, ordinary produced-army gameplay, fresh-copy validation, the normal full regression matrix and 1280×720/1920×1080 layout review. Starting forces, credits, costs, resource amounts, production timing and the 90-second scripted assault retain their playable values. Fog of war, attack-move, building groups, new gameplay types, strategic AI, multiplayer and movement investigation remain outside scope. **Milestone 9 is not started or authorized by this task.**
