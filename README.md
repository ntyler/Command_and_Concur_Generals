...
**Current milestone: [Milestone 12 — Bulldozers and builder-driven construction](docs/milestone-12.md) is INCOMPLETE.** Builder Assault is implemented; the original matrix’s four builder/earned-opening executions passed 710 checks. Final collection is complete: the original matrix finished 68/68 executions, 65 passing, 8814 checks, 14 failures, exit 1. The recorded graphical Attack Move group-3 failure remains of unknown cause: its earliest missing observation is accumulated selected IDs before Ctrl+3. Criterion 7 is NOT VERIFIED. Final targeted compatibility coverage passes 1030 checks (UI 113/116, groups 82/82, builder 314/323), including modifier release, builder-to-army transitions and mixed-group Q filtering; these passes do not reconstruct the original occurrence. Both failed movement-stress executions remain separately deferred and unresolved. This is the intended builder-based player experience at `res://scenes/builder_assault.tscn`. The [Milestone 11 baseline](docs/milestone-11.md) remains complete for its feature requirements. No human playtest occurred. Power generation is not implemented or begun.
