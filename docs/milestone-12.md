...
Status: **INCOMPLETE. Acceptance criterion 7: NOT VERIFIED.** The bounded control-group compatibility continuation is finished (2026-09-09). Current targeted interface checks pass, but the original graphical failure does not record cumulative selection before Ctrl+3, stored membership, or restored IDs. Its earliest unverified stage is valid accumulated selection before assignment; no production defect or invalid fixture assumption is established. Passing observations do not reconstruct that missing state. Full movement-regression acceptance also remains unpassed, separately under the existing deferral. Milestone 12 retains its existing identifier. Power generation is not implemented or begun.
...
| 7. Legacy scenes and existing interface | **NOT VERIFIED.** Legacy construction, production, economy, combat, HUD, tactical and groups suites pass, but original graphical paid-army group assignment/recall failed before Attack Move dispatch. Its cause remains unknown after passing focused observations; clean existing-interface compatibility cannot be claimed. |
...
**Disposition: original cause UNKNOWN; criterion 7 NOT VERIFIED.** No supported gameplay repair or invalid original input assumption was found. The current test changes improve failure localization and add compatibility coverage; they are not a repair of the original occurrence.
...
Final documentation checks pass **97 local links, including six anchors**, and **`git diff --check` exits 0**. Outcomes are saved in `compatibility-checks.json` after this report, README and roadmap are saved. The check also verifies the retained capture inventory and the one inspected current capture. No Godot process is launched by that check.
...
**Outstanding M12 requirement:** the original occurrence lacks an observation of the complete selected IDs before Ctrl+3, beginning with accumulation across Shift-clicks. If that stage had the expected [9,10,1,2,3], the next missing observation would be raw stored IDs immediately after assignment, followed by recall input/focus and restored IDs. Current source does not reproduce the failure in this bounded continuation; no defect or invalid original fixture assumption can be attributed from the retained evidence. It is not established as a movement limitation, pre-existing issue, builder regression or corrected fixture. Therefore M12 is **INCOMPLETE**, and criterion 7 is **NOT VERIFIED**, despite passing targeted compatibility checks. No open-ended native-input, historical engine or movement investigation is planned by this report. Full movement/Milestone 5 regression acceptance is separately unresolved under the existing deferral. Power remains future work and was not begun.
